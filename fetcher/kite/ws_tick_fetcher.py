...
class KT(KiteTicker):
    ...
    def _create_connection(self, url, **kwargs):
        wsstoken = self.config.get("wsstoken")
        ...
        url = 'wss://ws.zerodha.com/?api_key=kitefront&user_id={}&enctoken={}&uid=1624852235716&user-agent=kite3-web&version=2.9.1'.format(
            self.config.get("username"), wsstoken)
...
def on_ticks(ws, ticks):
    ...
    if os.path.exists("instrument_touch.txt") and config.get("subscribe_current_positions"):
        pos = ws.order.get_positions().json()
        ws.order.write_positions_tofile(pos, filename)
        os.remove("instrument_touch.txt")
